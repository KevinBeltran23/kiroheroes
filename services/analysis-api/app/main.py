...
from .firebase_client import download_video, update_job, update_session, upload_artifact, write_result
...
def analyze(request: AnalyzeRequest) -> dict[str, str]:
    try:
        update_job(request.jobId, {"status": "processing", "startedAt": firestore.SERVER_TIMESTAMP})
        update_session(request.sessionId, {"status": "processing"})

        with tempfile.TemporaryDirectory() as temp_dir:
            work_dir = Path(temp_dir)
            video_path = download_video(request.inputVideoPath, work_dir / "input-video")
            ...
            thumbnail_path = (
                f"users/{request.userId}/sessions/{request.sessionId}/artifacts/thumbnail.jpg"
            )
        ...
        safe_message = "The video could not be processed. Check framing and try a shorter clip."
        update_job(
            request.jobId,
            {
                "status": "failed",
