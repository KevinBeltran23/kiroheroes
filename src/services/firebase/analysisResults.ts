...
export async function createMockAnalysisResult(input: {
  ...
  await addDoc(collection(db, ANALYSIS_RESULTS_COLLECTION), {
    ...
    rightHand: {
      label: 'arm-heavy',
      explanation: 'Visible motion suggests the stroke is primarily driven from the elbow and forearm. (estimated arm: 55%, wrist: 25%)',
      fingerPct: 20,
      wristPct: 25,
      armPct: 55,
      compactness: 0.4,
    ...
    leftHand: {
      label: 'wrist-break',
      explanation: 'Visible motion suggests the stroke is primarily driven by rotational wrist movement. (estimated arm: 18%, wrist: 62%)',
      fingerPct: 20,
      wristPct: 62,
      armPct: 18,
      compactness: 0.6,
    },
    overallSummary: "Right hand: 'arm-heavy'. Left hand: 'wrist-break'. The two hands are showing different approach patterns, which may reflect technique differences or grip asymmetry.",
    artifactPaths: {},
    isMock: true,
